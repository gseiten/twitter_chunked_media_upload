...
cred = {
    "oauth_token": "729567090512830464-XchZ5L3A2UfmWw0p1kt4QsjXzF79fsu",
    "oauth_token_secret": "IB1dAwIjQzu93m77QPwW80iidSLVVF4npojBXwVZi1YWY",
}
...
def init(file_name):
    ...
    response = requests.post(url, headers=headers, files=mform_data)
    media_details = json.loads(response.text)
    print("\nINIT: ", media_details, "\n")
    # INIT:  {'expires_after_secs': 86399, 'media_id_string': '1187350127813390336', 'media_id': 1187350127813390336}
...
def status(media_id):
    ...
    auth_params = build_params(extras={"oauth_token": cred['oauth_token']})
    headers = make_oauth(url, "get", auth_params, args=q_args)
    url = "https://upload.twitter.com/1.1/media/upload.json?command=STATUS&media_id={}".format(media_id)
...
def media_tweet(media_id):
    ...
    q_args = {
        "status": "chunk upload image",
        "media_ids": media_id
    }
    headers = make_oauth(url, "post", auth_params, args=q_args)
    url += '?' + urlencode(q_args)
    response = requests.post(url, headers=headers)
    print("TWEET: ", response.text)
...
def chunked_upload(file_name=None):
    ...
    file_name = "fbtest3.mp4"
    media_id = init(file_name)
    segment_status = append(file_name, media_id)
    upload_details = finalize(media_id)
    tweet = media_tweet(media_id)
